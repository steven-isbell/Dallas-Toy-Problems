...
function dupe(arr, val) {
  return arr.find(
    post => post.text === val.text && post.username === val.username
  );
}

// return unique posts array
function cleanPosts(arr) {
  const unique = [];
  for (let i = 0; i < arr.length; i++) {
    if (!dupe(unique, arr[i])) {
      unique.push(arr[i]);
    }
  }
  return unique;
...
// store unique posts array
const newPosts = cleanPosts(posts);

const postToAdd = {
  id: 20,
  username: "ljeffs6",
  text:
    "Aenean lectus. Pellentesque eget nunc. Donec quis orci eget orci vehicula condimentum.\n\nCurabitur in libero ut massa volutpat convallis. Morbi odio odio, elementum eu, interdum eu, tincidunt in, leo. Maecenas lobortis est.",
  date: "2017-09-25 19:30:55"
};

// add post to array
function addPost(post) {
  if (!dupe(newPosts, post)) {
    newPosts.push(post);
    return newPosts;
  } else {
